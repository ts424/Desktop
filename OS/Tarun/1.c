...
int main() {
    ...
    for (i = 0; i < num_processes; i++) {
        printf("Enter the arrival time and burst time of each process: %d: ", i + 1);
    ...
    for (i = 0; i < num_processes; i++) {
        ...
        avg_wait_time =avg_wait_time + current_time - processes[max_burst_time_index].arrival_time;
        current_time =current_time + processes[max_burst_time_index].burst_time;
        avg_turnaround_time =avg_turnaround_time + current_time - processes[max_burst_time_index].arrival_time;

        processes[max_burst_time_index].burst_time = 0;
    }

    printf("\n\nAverage waiting time: %.2f", avg_wait_time / num_processes);
